...
// Runs a non-QDQ RotaryEmbedding model on HTP and compares output to CPU EP.
...
// Test RotaryEmbedding with 4D input [B, NH, S, HS], non-interleaved, full rotation
...
// Test RotaryEmbedding with 4D input, interleaved mode
...
// Test RotaryEmbedding with 4D input, partial rotation
TEST_F(QnnHTPBackendTests, RotaryEmbedding_4D_PartialRotation) {
  constexpr int64_t batch_size = 1;
  constexpr int64_t num_heads = 2;
  constexpr int64_t seq_len = 4;
  constexpr int64_t head_size = 8;
  constexpr int64_t rotary_dim = 4;  // Partial rotation (half of head_size)
...
// Test RotaryEmbedding with 3D input [B, S, NH*HS]
...
// Test RotaryEmbedding with 3D input and interleaved mode
...
// Test RotaryEmbedding with position_ids input
TEST_F(QnnHTPBackendTests, RotaryEmbedding_WithPositionIds) {
  ...
  std::vector<float> cos_cache = GetFloatDataInRange(-1.0f, 1.0f, seq_len * (rotary_dim / 2));
  std::vector<float> sin_cache = GetFloatDataInRange(-1.0f, 1.0f, seq_len * (rotary_dim / 2));
  std::vector<int64_t> position_ids = {0, 1, 2, 3, 0, 1, 2, 3};  // [B, S]
...
// Test RotaryEmbedding with 3D cos/sin caches [B, S, rotary_dim/2]
TEST_F(QnnHTPBackendTests, RotaryEmbedding_3D_Caches) {
  constexpr int64_t batch_size = 2;
  constexpr int64_t num_heads = 2;
  constexpr int64_t seq_len = 4;
  constexpr int64_t head_size = 8;
...
// Test RotaryEmbedding with default rotary_embedding_dim (0 means use full head_size)
...
// Test RotaryEmbedding with larger dimensions
...
// Test RotaryEmbedding with FP16 data type
