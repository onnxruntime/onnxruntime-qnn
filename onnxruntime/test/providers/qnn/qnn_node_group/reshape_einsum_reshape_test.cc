...
GetTestModelFn BuildReshapeEinsumReshapeFloatTestCase() {
  return [](ModelTestBuilder& builder) -> void {
    ...
    NodeArg* reshape1_shape = builder.Make1DInitializer<int64_t>({1, 32, 32, 2, 2, 8});
    NodeArg* reshape1_out = builder.MakeIntermediate();
    builder.AddNode("Reshape", {input, reshape1_shape}, {reshape1_out});

    // Einsum: equation "nhwpqc->nchpwq" (equivalent to Transpose with perm [0, 5, 1, 3, 2, 4])
    NodeArg* einsum_out = builder.MakeIntermediate();
    Node& einsum = builder.AddNode("Einsum", {reshape1_out}, {einsum_out});
    einsum.AddAttribute("equation", "nhwpqc->nchpwq");

    // Reshape: (1, 8, 32, 2, 32, 2) -> (1, 8, 64, 64)
    NodeArg* reshape2_shape = builder.Make1DInitializer<int64_t>({1, 8, 64, 64});
    NodeArg* output = builder.MakeOutput();
    builder.AddNode("Reshape", {einsum_out, reshape2_shape}, {output});
  };
}
