...
GetQDQTestCaseFn BuildLPBQMatMulTestCase() {
  return [](ModelTestBuilder& builder) -> void {
    // Define the test case for LPBQMatMul fusion here
    ...
    builder.graph_->set_name("lpbqmatmul_fusion_graph");
    ...
    // DequantizeLinear for Scale (per-channel)
    builder.MakeInitializer<uint8_t>("scale_dq_in", {blocks_per_axis, output_channels}, 1, 16);
    ...
    builder.MakeInitializer<float>("w_fp", {input_channels, output_channels}, -2.0f, 2.0f);
    ...
    // Output QDQ: matmul_out -> Q -> DQ -> output
    constexpr float out_scale = 0.00019595865160226822f;
    constexpr uint16_t out_zp = 31693;
    AddQDQNodePairWithOutputAsGraphOutput<uint16_t>(builder, "qdq_out", "matmul_out", out_scale, out_zp);
  };
}
