...
void AssertOpInQnnGraph(const std::filesystem::path& dump_dir,
                        const std::string& op,
                        size_t count) {
  // Sum occurrences of `op` across every QNN JSON graph file in `dump_dir`. A model that
  // produces multiple QNN partitions writes one JSON per partition, and we want the total.
  ...
  ASSERT_FALSE(json_paths.empty()) << "No QNN JSON graph files found in " << dump_dir;
  ...
  EXPECT_EQ(actual_count, count)
      ...
      << " across " << json_paths.size() << " JSON file(s) in " << dump_dir;
}
