...
// Covers the ctor WARNING that fires when enable_htp_cross_device_prepare=1 but the
// current build lacks cross-device-prepare support (QNN_HTP_CROSS_DEVICE_PREPARE_AVAILABLE
...
TEST_F(QnnUnit_ExecutionProviderTest, Ctor_EnableCrossDevicePrepareOnUnsupportedBuild_LogsWarning) {
  EpStubContext ctx;
  ctx.log_severity = ORT_LOGGING_LEVEL_VERBOSE;
  ctx.session_config[EPKey("enable_htp_cross_device_prepare")] = "1";
  auto factory = MakeFactory(ctx);
  EXPECT_NO_THROW({ auto ep = MakeEp(*factory, ctx); });
  ExpectLogged(ctx, ORT_LOGGING_LEVEL_WARNING, "HTP cross device prepare is not available in current build.");
}
...
TEST_F(QnnUnit_ExecutionProviderTest, Ctor_EnableCrossDevicePrepareDefault_NoWarningLogged_Succeeds) {
  EpStubContext ctx;
  ctx.log_severity = ORT_LOGGING_LEVEL_VERBOSE;
  // enable_htp_cross_device_prepare defaults to false, so the "not available" warning
  // (which only fires when the option was explicitly requested) must not be logged.
  auto factory = MakeFactory(ctx);
  EXPECT_NO_THROW({ auto ep = MakeEp(*factory, ctx); });
  for (const auto& rec : ctx.log_records) {
    EXPECT_EQ(rec.message.find("HTP cross device prepare is not available in current build."), std::string::npos)
