...
// Function that builds a QDQ model with a GroupNormalization operator.
template <typename QuantType>
static GetTestQDQModelFn<QuantType> BuildQDQGroupNormTestCase(const TestInputDef<float>& input_def,
  ...
  return [input_def, scale_def, bias_def, attrs,
    ...
    auto* group_norm_output = builder.MakeIntermediate();
    Node& group_norm_node = builder.AddNode("GroupNormalization", {input_qdq, scale_qdq, bias_qdq},
                                            {group_norm_output});
    for (const auto& attr : attrs) {
      group_norm_node.AddAttributeProto(attr);
    }

    // Add group_norm_output -> Q -> output_u8
    AddQDQNodePairWithOutputAsGraphOutput<QuantType>(builder, group_norm_output, output_qparams[0].scale,
                                                     output_qparams[0].zero_point, use_contrib_qdq);
  };
}

/**
 * Runs a GroupNormalization model on the QNN HTP backend. Checks the graph node assignment, and that inference
 ...
 * \param attrs The node's attributes. The valid attributes for GroupNormalization are 'epsilon' and 'num_groups'.
 * \param expected_ep_assignment How many nodes are expected to be assigned to QNN (All, Some, or None).
 */
template <typename QuantType = uint8_t>
static void RunGroupNormQDQTest(const TestInputDef<float>& input_def,
                                const TestInputDef<float>& scale_def,
                                const TestInputDef<float>& bias_def,
                                const std::vector<ONNX_NAMESPACE::AttributeProto>& attrs,
  ...
  // Runs model with DQ-> GroupNormalization -> Q and compares the outputs of the CPU and QNN EPs.
  TestQDQModelAccuracy(BuildOpTestCase<float>("GroupNormalization", {input_def, scale_def, bias_def}, {}, attrs),
                       BuildQDQGroupNormTestCase<QuantType>(input_def, scale_def, bias_def, attrs, use_contrib_qdq),
                       provider_options,
                       18,
...
TEST_F(QnnHTPBackendTests, GroupNormU8) {
...
TEST_F(QnnHTPBackendTests, GroupNormU16) {
  std::vector<float> input_data = {3.21289f, -5.9981f, -1.72799f, 6.27263f, 3.36205f, -1.93515f, -5.40113f, 3.75648f, 6.15357f,
                                   -5.25769f, 2.73637f, -0.901382f, -6.55612f, 1.99497f, -4.79228f, 2.69813f, 8.3064f, 0.0362501f};
  std::vector<float> scale_data = {-0.148738f, -1.45158f};
  std::vector<float> bias_data = {-2.2785083772f, 2.3338717017f};
...
// Test 8-bit QDQ GroupNormalization with an input of rank 3 with N != 1,
// which requires wrapping the QNN GroupNormalization op with reshapes.
...
// Test 16-bit QDQ GroupNormalization with an input of rank 3 with N != 1,
// which requires wrapping the QNN GroupNormalization op with reshapes.
