...
TEST_F(QnnGPUBackendTests, get_allocator_qnn_gpu_shared) {
  ...
  Ort::MemoryInfo info_qnn_gpu_shared("QnnGpuShared",
                                      OrtMemoryInfoDeviceType_GPU,
                                      OrtDevice::VendorIds::QUALCOMM,
                                      0,
                                      OrtDeviceMemoryType_HOST_ACCESSIBLE,
                                      0,
                                      OrtDeviceAllocator);
  Ort::Allocator qnn_gpu_shared_allocator(session, info_qnn_gpu_shared);

  auto allocator_info = qnn_gpu_shared_allocator.GetInfo();
  ASSERT_EQ(allocator_info, info_qnn_gpu_shared);
...
TEST_F(QnnGPUBackendTests, io_binding_qnn_gpu_shared) {
  RegisteredEpDeviceUniquePtr registered_ep_device;
  {
    ...
    Ort::Allocator qnn_gpu_shared_allocator(session, info_qnn_gpu_shared);
    auto allocator_info = qnn_gpu_shared_allocator.GetInfo();
    ASSERT_EQ(info_qnn_gpu_shared, allocator_info);
    ...
    // Create an OrtValue tensor backed by data on DX12 memory
    Ort::Value bound_x = Ort::Value::CreateTensor(info_qnn_gpu_shared, reinterpret_cast<float*>(input_data.get()), x_values.size(),
                                                  x_shape.data(), x_shape.size());
    ...
    // Create an OrtValue tensor backed by data on DX12 memory
    Ort::Value bound_y = Ort::Value::CreateTensor(info_qnn_gpu_shared, reinterpret_cast<float*>(output_data.get()),
                                                  expected_y.size(), expected_y_shape.data(), expected_y_shape.size());
    ...
    // This is when we can not allocate an OrtValue due to unknown dimensions
    {
      binding.BindOutput("Y", info_qnn_gpu_shared);
      session.Run(Ort::RunOptions(), binding);
    }
