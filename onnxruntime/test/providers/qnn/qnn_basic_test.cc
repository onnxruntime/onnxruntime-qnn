...
TEST_F(QnnHTPBackendTests, TempDumpDlcTest) {
  ...
  InitNHWCResizeModel("/local/mnt/workspace/scratch/models/efficient_l2_cubic/model.onnx",
                      TestBackend::Htp,  // backend
                      TestBackend::Ir);  // serializer backend
...
TEST_F(QnnHTPBackendTests, TestTempModel) {
  const std::string path{"/local/mnt/workspace/repo/onnxruntime-qnn-ep/QnnHTPBackendTests_ResizeU8_2xCubicHalfPixel/dumped_f32_model.onnx"};
  ProviderOptions provider_options;
  provider_options["backend_type"] = "htp";
  provider_options["offload_graph_io_quantization"] = "0";
  ...
  verification_params.fp32_abs_err = 1e-5f;
