...
static void RunIrStatefulLSTMCompositionTest(uint32_t seq_len, uint32_t batch_size,
  ...
  if (has_reset) {
    // Populate every preceding optional input so the BOOL reset remains at
    // ONNX input 8 during placeholder-op type inference.
    builder.MakeInitializer<int32_t>("sequence_lens", {batch_size},
                                     std::vector<int32_t>(batch_size, static_cast<int32_t>(seq_len)));
    input_names[4] = "sequence_lens";
    builder.MakeInitializer<float>("P", {1, 3 * hidden_size}, std::vector<float>(3 * hidden_size, 0.0f));
    input_names.push_back("P");
    builder.MakeInput<bool>("slstm_reset", {}, {false});
    input_names.push_back("slstm_reset");  // QNN LSTM in[24]
  }
...
static void RunIrStatefulGRUCompositionTest(uint32_t seq_len, uint32_t batch_size,
                                            uint32_t input_size, uint32_t hidden_size,
                                            bool has_B, bool has_H, bool has_reset = false) {
  ProviderOptions provider_options;
  provider_options["backend_type"] = "ir";
  ...
  if (has_reset) {
    // Populate every preceding optional input so the BOOL reset remains at
    // ONNX input 6 during placeholder-op type inference.
    builder.MakeInitializer<int32_t>("sequence_lens", {batch_size},
                                     std::vector<int32_t>(batch_size, static_cast<int32_t>(seq_len)));
    input_names[4] = "sequence_lens";
    builder.MakeInput<bool>("sgru_reset", {}, {false});
    input_names.push_back("sgru_reset");  // QNN GRU in[14]
  }
...
TEST_F(QnnIRBackendTests, StatefulLSTM_FP32_IR_with_reset) {
  RunIrStatefulLSTMCompositionTest(5, 1, 3, 4,
                                   /*has_B=*/true, /*has_H=*/true, /*has_C=*/true,
                                   /*has_reset=*/true);
}
...
TEST_F(QnnIRBackendTests, StatefulGRU_FP32_IR_with_reset) {
  RunIrStatefulGRUCompositionTest(5, 1, 3, 4,
                                  /*has_B=*/true, /*has_H=*/true,
                                  /*has_reset=*/true);
}
