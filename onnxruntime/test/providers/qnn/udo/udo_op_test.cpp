...
// register the custom op in onnx domain which is already registered by QNN EP
...
TEST_F(QnnHTPBackendTests, DISABLED_UDO_Op_Increment) {
  auto input = TestInputDef<float>({1, 32}, false, -1.0f, 1.0f);
...
TEST_F(QnnHTPBackendTests, DISABLED_UDO_Op_Increment_with_constant) {
  auto input = TestInputDef<float>({1, 32}, false, -1.0f, 1.0f);
  std::filesystem::path path = getLibPath("htp");
  RunOpTestOnHTP("Increment",
                 {input},
