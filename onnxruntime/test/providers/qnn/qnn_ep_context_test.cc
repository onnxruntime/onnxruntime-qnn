...
#if !defined(ORT_MINIMAL_BUILD) && BUILD_QNN_EP_STATIC_LIB
TEST(QnnTryGetMaxSpillFillSizeTest, NonConsecutiveMainContexts) {
  // Main contexts at indices {0, 3, 5}. Non-main positions carry LARGER max_size
  // values so a regression that reads graphs[idx] instead of
  // graphs[main_context_pos_list[idx]] will pick them up.
  struct Spec {
    int64_t main_context;
    int64_t max_size;
  };
  const Spec specs[6] = {
      {1, 100000},
      {0, 999999},  // non-main; would hijack the max if the indirection is dropped
      {0, 888888},
      {1, 200000},  // largest among main contexts
      {0, 777777},
      {1, 150000},
  };
  ...
  // Own the Model/GraphViewer/EpGraph for each graph past the call to TryGetMaxSpillFillSize.
  std::vector<std::unique_ptr<onnxruntime::Model>> models;
  std::vector<std::unique_ptr<onnxruntime::GraphViewer>> viewers;
  std::vector<std::unique_ptr<onnxruntime::EpGraph>> ep_graphs;
  std::vector<const OrtGraph*> graphs;

  ONNX_NAMESPACE::TypeProto float_tensor;
  float_tensor.mutable_tensor_type()->set_elem_type(ONNX_NAMESPACE::TensorProto::FLOAT);
  float_tensor.mutable_tensor_type()->mutable_shape()->add_dim()->set_dim_value(1);

  for (int i = 0; i < 6; ++i) {
    auto model = std::make_unique<onnxruntime::Model>(
        "ep_ctx_model_" + std::to_string(i), /*is_onnx_domain_only*/ false,
        onnxruntime::ModelMetaData(), onnxruntime::PathString(),
        onnxruntime::IOnnxRuntimeOpSchemaRegistryList(), domain_to_version,
        std::vector<ONNX_NAMESPACE::FunctionProto>(), logger);
    onnxruntime::Graph& graph = model->MainGraph();

    auto& in_arg = graph.GetOrCreateNodeArg("in_" + std::to_string(i), &float_tensor);
    auto& out_arg = graph.GetOrCreateNodeArg("out_" + std::to_string(i), &float_tensor);

    onnxruntime::NodeAttributes attrs;
    ONNX_NAMESPACE::AttributeProto main_ctx_attr;
    main_ctx_attr.set_name("main_context");
    main_ctx_attr.set_type(ONNX_NAMESPACE::AttributeProto::INT);
    main_ctx_attr.set_i(specs[i].main_context);
    attrs["main_context"] = main_ctx_attr;
    ONNX_NAMESPACE::AttributeProto max_size_attr;
    max_size_attr.set_name("max_size");
    max_size_attr.set_type(ONNX_NAMESPACE::AttributeProto::INT);
    max_size_attr.set_i(specs[i].max_size);
    attrs["max_size"] = max_size_attr;

    graph.AddNode("ep_ctx_node_" + std::to_string(i), "EPContext",
                  "regression test EPContext node",
                  std::vector<onnxruntime::NodeArg*>{&in_arg},
                  std::vector<onnxruntime::NodeArg*>{&out_arg},
                  &attrs, kMSDomain);
    graph.SetInputs({&in_arg});
    graph.SetOutputs({&out_arg});
    ASSERT_STATUS_OK(graph.Resolve());

    auto viewer = std::make_unique<onnxruntime::GraphViewer>(graph);
    std::unique_ptr<onnxruntime::EpGraph> ep_graph;
    ASSERT_STATUS_OK(onnxruntime::EpGraph::Create(*viewer, ep_graph));
    graphs.push_back(ep_graph.get());

    models.push_back(std::move(model));
    viewers.push_back(std::move(viewer));
    ep_graphs.push_back(std::move(ep_graph));
  }

  std::vector<int> main_context_pos_list = {0, 3, 5};
  int64_t max_spill_fill_size = 0;
  Ort::Status status = onnxruntime::qnn::TryGetMaxSpillFillSize(
      graphs.data(), Ort::GetApi(),
      static_cast<uint32_t>(main_context_pos_list.size()),
      max_spill_fill_size, main_context_pos_list);
  ASSERT_TRUE(status.IsOK()) << status.GetErrorMessage();

  // Expected: largest max_size among the 3 main contexts (200000 at position 3).
  // The buggy code reads graphs[0..2] and returns 999999.
  EXPECT_EQ(max_spill_fill_size, 200000)
      << "TryGetMaxSpillFillSize read non-main-context graphs; see commit e6310479ac.";
  EXPECT_EQ(main_context_pos_list[0], 3);
  std::vector<int> remaining{main_context_pos_list[1], main_context_pos_list[2]};
  std::sort(remaining.begin(), remaining.end());
  EXPECT_EQ(remaining, (std::vector<int>{0, 5}));
}
