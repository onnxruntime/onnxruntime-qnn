...
// Create a model with Crop (unsupported on HTP) + Add (quantized)
// input1 -> Crop -> Q -> DQ ----
//                               |
...
static GetTestModelFn BuildGraphWithQAndNonQ(bool single_ep_node = true) {
  return [single_ep_node](ModelTestBuilder& builder) {
    // Create Crop node (not supported on HTP, runs on CPU)
    std::vector<float> data(200 * 200, 1.0f);
    NodeArg* input1 = MakeTestInput(builder, TestInputDef<float>({1, 1, 200, 200}, false, data));

    auto* add1_output = builder.MakeIntermediate();
    Node& crop_node1 = builder.AddNode("Crop", {input1}, {add1_output});
    crop_node1.AddAttribute("border", std::vector<int64_t>{0, 0, 0, 0});
    crop_node1.AddAttribute("scale", std::vector<int64_t>{1, 1, 200, 200});
    ...
    NodeArg* add2_input2 = MakeTestInput(builder, TestInputDef<float>({1, 1, 200, 200}, true, data));
    ...
    if (single_ep_node) {
      ...
      NodeArg* add3_ini_input2 = MakeTestInput(builder, TestInputDef<float>({1, 1, 200, 200}, true, data));

      auto* add3_output = builder.MakeIntermediate();
      Node& crop_node3 = builder.AddNode("Crop", {add3_ini_input2}, {add3_output});
      crop_node3.AddAttribute("border", std::vector<int64_t>{0, 0, 0, 0});
      crop_node3.AddAttribute("scale", std::vector<int64_t>{1, 1, 200, 200});
      ...
      NodeArg* add4_input2 = MakeTestInput(builder, TestInputDef<float>({1, 1, 200, 200}, true, data));
...
// Test that models with 1 non-quantized Crop node and 1 quantized Add node can still generate the context binary
// The generated Onnx model has 1 Crop node and 1 EPContext node
...
// Test that models with 2 non-quantized Crop nodes and 2 quantized Add nodes can still generate the context binary
// The generated Onnx model has 2 Crop nodes and 1 EPContext nodes
...
// Set the session option "ep.context_model_external_initializers_file_name" so Crop (which fallback on CPU)
...
// so Crop (which fallback on CPU) will NOT dump initializer data to external file
