...
// Test that Mod with dynamic divisor.
TEST_F(QnnHTPBackendTests, threlu_model_test) {
  const std::string path{"/local/mnt/workspace/onnxruntime_mlg_1/test_models/theshoulded_relu.onnx"};
  ProviderOptions provider_options;
  ...
  const std::vector<int64_t> input_shape{3, 5};
  auto input_tokens = rand_gen_.Uniform<float>(input_shape, -1.0f, 1.0f);

  OrtValue input_token_value;
  CreateMLValue<float>(allocator, input_shape, std::move(input_tokens), &input_token_value);
  feeds.insert(std::make_pair("x", input_token_value));
