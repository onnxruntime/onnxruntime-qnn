...
static void RunGatherBlockQuantizedUnpackedInt4Test(
  ...
  auto model_builder = [&params](ModelTestBuilder& builder) {
    ...
    // Int4x2 has no Ort::TypeToTensorType specialization, so it can only be used as a graph
    // initializer (via MakeInitializer directly), not as a dynamic input (MakeTestInput's
