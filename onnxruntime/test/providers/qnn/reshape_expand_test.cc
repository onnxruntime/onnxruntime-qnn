...
// Test that QDQ Reshape with mismatched DQ input type (uint4) and Q output type (uint8)
...
TEST_F(QnnHTPBackendTests, Reshape_QDQ_DtypeMismatch_NotSelected) {
  auto build_model = [](ModelTestBuilder& builder) {
    ...
    // DQ input type (uint16) differs from Q output type (uint8), so DropQDQNodeGroupSelector
    // should reject this pattern.
    ...
    // Output Q/DQ with uint8 — deliberate type mismatch
