...
TEST_F(QnnHTPBackendTests, StatefulGRU_QDQ_u8_sanity_forward) {
  std::string direction = "forward";
  uint32_t num_direction = 1;
  uint32_t batch_size = 3;
  uint32_t hidden_size = 4;
  ...
  RunHtpQDQStatefulGRUOpTest<uint8_t>(
      ...
      direction,                                                                               // direction
      hidden_size,                                                                             // hidden_size
      // The INT32 bias DequantizeLinear node is executed on CPU. QNN receives
      // its result and owns the StatefulGru subgraph.
