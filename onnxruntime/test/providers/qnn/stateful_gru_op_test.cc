...
/*
  ...
  HTP constraints:
  - Native INT8/INT16 Gru is forward-only per HtpOpDefSupplement. A bidirectional QDQ group is
    fp-degraded on QNN, while a non-QDQ quantized StatefulGru is rejected.
...
void _BuildStatefulGRUTestCase(ModelTestBuilder& builder,
  ...
  input_names.reserve(has_reset ? 7 : 6);
  ...
  if (B_def) {
    if constexpr (kIsU8 || kIsU16) {
      // QNN HTP GRU requires its six bias slots to be SFIXED_POINT_32. The ONNX
      // block-op interface remains float32 after DQ, but its Q input must be INT32.
      // Use the same int32-initializer + DequantizeLinear convention as the other
      // QNN RNN tests instead of a Q/DQ uint8/uint16 pair.
      const float x_scale = GetTestInputQuantParams<InputType>(X_def).scale;
      const float w_scale = GetTestInputQuantParams<InputType>(W_def).scale;
      input_names.push_back(MakeTestQDQBiasInput(builder, "B", B_def->get(), x_scale * w_scale));
    } else {
  ...
  // reset (in[6], BOOL scalar). Do not add Q/DQ: reset is always a raw BOOL_8 signal.
  if (has_reset) {
  ...
  // FP32: use the BlockOp-defined float32 type. QDQ: declare float32 value_info for the intermediate tensors
...
static GetTestQDQModelFn<InputQType> BuildQDQStatefulGRUTestCase(
  ...
  return [X_def, W_def, R_def, B_def, H_def,
          has_Y, has_Y_h, direction, hidden_size, linear_before_reset, has_reset](
...
// Runs a StatefulGru model on the QNN HTP backend using the BlockOp-defined FP32 interface.
static void RunHtpFp32StatefulGRUOpTest(
    const TestInputDef<float>& X_def,
    const TestInputDef<float>& W_def,
    const TestInputDef<float>& R_def,
    const std::optional<std::reference_wrapper<TestInputDef<float>>> B_def,
  ...
  RunQnnOnlyStatefulGRUModel(BuildStatefulGRUTestCase<float>(X_def, W_def, R_def, B_def, H_def,
                                                             has_Y, has_Y_h, direction, hidden_size, linear_before_reset),
                             provider_options,
...
// ============================================================
// HTP FP32 Tests
// QNN_COMMON_ERROR_MEM_ALLOC at graph finalization affects all known HTP devices
// (v68, v73, v81) because their firmware lacks StatefulGru persistent-state support.
// Re-enable once that firmware support is available.
// ============================================================

TEST_F(QnnHTPBackendTests, StatefulGRU_Fp32_sanity_forward) {
...
TEST_F(QnnHTPBackendTests, StatefulGRU_Fp32_sanity_forward_wo_B) {
  std::string direction = "forward";
  uint32_t num_direction = 1;
  uint32_t batch_size = 3;
  uint32_t hidden_size = 4;
...
TEST_F(QnnHTPBackendTests, StatefulGRU_QDQ_u8_fallback_forward_lbr0) {
  std::string direction = "forward";
  uint32_t num_direction = 1;
  uint32_t batch_size = 3;
  uint32_t hidden_size = 4;
...
TEST_F(QnnHTPBackendTests, StatefulGRU_QDQ_u8_fallback_forward_lbr0_with_reset) {
  std::string direction = "forward";
  uint32_t num_direction = 1;
  uint32_t batch_size = 3;
  uint32_t hidden_size = 4;
  ...
  RunHtpQDQStatefulGRUOpTest<uint8_t>(
      ...
      hidden_size,                                                                             // hidden_size
      ExpectedEPNodeAssignment::All,
      0,      // linear_before_reset: force fp fallback for the u8 combo
      true);  // has_reset
}

TEST_F(QnnHTPBackendTests, StatefulGRU_QDQ_u8_native_forward_lbr1) {
  std::string direction = "forward";
  uint32_t num_direction = 1;
  uint32_t batch_size = 3;
  uint32_t hidden_size = 4;
  ...
  RunHtpQDQStatefulGRUOpTest<uint8_t>(
      ...
      hidden_size,                                                                             // hidden_size
      ExpectedEPNodeAssignment::All,
      1);  // linear_before_reset: native INT8 Gru
}

TEST_F(QnnHTPBackendTests, StatefulGRU_QDQ_u16_sanity_forward) {
  std::string direction = "forward";
  uint32_t num_direction = 1;
  uint32_t batch_size = 3;
  uint32_t hidden_size = 4;
...
TEST_F(QnnHTPBackendTests, StatefulGRU_Fp32_bidirectional) {
...
TEST_F(QnnHTPBackendTests, StatefulGRU_Fp32_with_reset) {
  ProviderOptions provider_options;
  provider_options["backend_type"] = "htp";
...
TEST_F(QnnHTPBackendTests, StatefulGRU_Fp32_reset_restores_initial_state) {
  QNN_SKIP_TEST_ON_LINUX_X86_64("qti_aisw StatefulGru requires HTP hardware; not supported on the x86_64 simulator.");
  SKIP_HTP_TEST_ON_ARCH_LESS_THAN_OR_EQUAL_TO(QNN_HTP_DEVICE_ARCH_V68);
