...
TEST_F(QnnABIHTPBackendTests, OffloadGraphIoQuantizationTensorNameAliasing) {
  ProviderOptions provider_options;
#if defined(_WIN32)
  provider_options["backend_path"] = "QnnHtp.dll";
#else
  ...
  const std::filesystem::path dump_dir = "test_tensor_name_alias_";
  ...
  onnxruntime::Model qdq_model("OffloadGraphIoQuantizationTensorNameAliasing", false, ModelMetaData(), PathString(),
                               IOnnxRuntimeOpSchemaRegistryList(), domain_to_version, {},
                               logging_manager.DefaultLogger());
  ...
  // Find and parse the generated JSON file
  ...
  // Verify aliased names appear in DLC (not internal names like "q_out", "dq1_out")
  EXPECT_TRUE(tensor_names.count(expected_input_name) > 0)
      << "Expected input '" << expected_input_name << "' not found.";
  EXPECT_TRUE(tensor_names.count(expected_output_name) > 0)
