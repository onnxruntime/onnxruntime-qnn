...
std::vector<float> GetSequentialFloatDataABI(const std::vector<int64_t>& shape, float start, float step) {
  if (shape.empty()) {
    return {};
  }
...
Status QnnHTPBackendTests::QueryQnnPlatformAttributesDirectly(QnnHTPBackendTests::QnnPlatformAttributes& out, const onnxruntime::logging::Logger& logger) {
  ...
  auto backendGetApiVersionFn = qnn_interface->QNN_INTERFACE_VER_NAME.backendGetApiVersion;Expand commentComment on lines R695 to R706ResolvedCode has comments. Press enter to view.
  ...
  if (platform_info_ptr->version == QNN_DEVICE_PLATFORM_INFO_VERSION_1) {
    ...
    for (uint32_t i = 0; i < p.numHwDevices; ++i) {
      ...
      if (htp_ext && htp_ext->devType == QNN_HTP_DEVICE_TYPE_ON_CHIP) {
        ...
        break;Expand commentComment on lines R759 to R767ResolvedCode has comments. Press enter to view.
      }
    }Expand commentComment on line R769ResolvedCode has comments. Press enter to view.
  } else {
