...
Ort::Status QnnBackendManager::LoadCachedQnnContextFromBuffer(
  ...
  RETURN_IF_ERROR(ExtractBackendProfilingInfo());
  context_created_ = true;
...
Ort::Status QnnBackendManager::ExtractBackendProfilingInfo() {
  if (ProfilingLevel::OFF == profiling_level_merge_ || ProfilingLevel::INVALID == profiling_level_merge_) {
    return Ort::Status();
  }
  ...
  if (num_events > 0) {
    ...
    if (!profiling_file_path_.empty()) {
      ...
      RETURN_IF(!outfile.is_open(), ("Failed to open profiling file: " + profiling_file_path_).c_str());
...
Ort::Status QnnBackendManager::ExtractProfilingEvent(
...
Ort::Status QnnBackendManager::ExtractProfilingEventBasic(
    QnnProfile_EventId_t profile_event_id,
    const std::string& eventLevel,
    std::ofstream& outfile,
    bool tracelogging_provider_ep_enabled) {
