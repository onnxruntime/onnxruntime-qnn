...
Status GetInputNodeUnits(const QnnModelWrapper& qnn_model_wrapper,
  ...
  // Get node inputs
  size_t num_inputs = 0;
  RETURN_STATUS_IF_ERROR(ort_api.Node_GetNumInputs(&node, &num_inputs), ort_api);
  std::vector<const OrtValueInfo*> inputs(num_inputs);
  RETURN_STATUS_IF_ERROR(ort_api.Node_GetInputs(&node, inputs.data(), inputs.size()), ort_api);
  ...
  for (size_t i = 0; i < num_inputs; ++i) {
    ...
    RETURN_STATUS_IF_ERROR(ort_api.ValueInfo_GetValueProducer(input_value_info, &input_node, nullptr),
                           ort_api);
    if (input_node != nullptr) {
      const auto input_node_unit_it = node_unit_map.find(input_node);
      if (input_node_unit_it == node_unit_map.end()) {
        return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "Input node has no OrtNodeUnit mapping.");
      }
      ...
      if (input_node_unit->UnitType() != OrtNodeUnit::Type::SingleNode) {
...
Status GetOutputNodeUnits(const QnnModelWrapper& qnn_model_wrapper,
                          const OrtNodeUnit& node_unit,
                          const std::unordered_map<const OrtNode*, const OrtNodeUnit*>& node_unit_map,
  ...
  RETURN_STATUS_IF_ERROR(ort_api.Node_GetNumOutputs(&node, &num_outputs), ort_api);
  std::vector<const OrtValueInfo*> outputs(num_outputs);
  RETURN_STATUS_IF_ERROR(ort_api.Node_GetOutputs(&node, outputs.data(), outputs.size()), ort_api);
  ...
  for (size_t i = 0; i < num_outputs; ++i) {
    ...
    RETURN_STATUS_IF_ERROR(ort_api.ValueInfo_GetValueNumConsumers(output_value_info, &num_consumers), ort_api);
    std::vector<const OrtNode*> output_nodes(num_consumers);
    std::vector<int64_t> output_consumer_indices(num_consumers);
    RETURN_STATUS_IF_ERROR(ort_api.ValueInfo_GetValueConsumers(output_value_info,
                                                               output_nodes.data(),
    ...
    if (!output_nodes.empty() && output_nodes[0] != nullptr) {
      ...
      if (output_node_unit_it == node_unit_map.end()) {
        return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "Output node has no OrtNodeUnit mapping.");
      }
      const OrtNodeUnit* output_node_unit = output_node_unit_it->second;

      if (output_node_unit->UnitType() != OrtNodeUnit::Type::SingleNode) {
    ...
    switch (attr_type) {
      ...
      case ORT_OP_ATTR_INT: {
        ...
        ORT_RETURN_IF_NOT(optional_int64.has_value(),
                          "Failed to get values from attr ", attr_name, " in op ", node_name, " to qnn_model_wrapper.");
        ORT_RETURN_IF_ERROR(AddQnnScalar<int32_t>(qnn_model_wrapper,
                                                  node_unit.Index(),
                                                  node_name,
      ...
      default: {
        return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "Failed to add scalar attr ", attr_name, " data_type ", attr_type,
...
std::unique_ptr<IQnnNodeGroup> UDOQDQFusion::TryFusion(
  ...
  Status status = GetInputNodeUnits(qnn_model_wrapper, udo_node_unit, node_to_node_unit, node_unit_to_qnn_node_group, input_node_units);
  if (!status.IsOK()) {
    return nullptr;
  }
