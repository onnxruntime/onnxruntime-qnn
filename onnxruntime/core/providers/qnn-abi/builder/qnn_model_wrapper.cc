...
Ort::Status QnnModelWrapper::CreateQnnGraph(const Qnn_ContextHandle_t& context,
  ...
  RETURN_IF(rt == QNN_COMMON_ERROR_SYSTEM_COMMUNICATION, "SSR detected during graph creation. Error code: 1007");

  if (rt != QNN_GRAPH_NO_ERROR || graph_ == nullptr) {
    rt = qnn_interface_.graphRetrieve(context, graph_name.c_str(), &graph_);
    RETURN_IF(rt != QNN_GRAPH_NO_ERROR || graph_ == nullptr, ("Failed to create Qnn graph: " + graph_name).c_str());
  }
...
Ort::Status QnnModelWrapper::ComposeQnnGraph(bool build_json_qnn_graph) {
  ...
  RETURN_IF(qnn_op_property_list_.empty(), "Empty Qnn op list, no graph to compose.");
