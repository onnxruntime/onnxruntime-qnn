...
Ort::Status GroupNormOpBuilder::IsOpSupported(QnnModelWrapper& qnn_model_wrapper,
  ...
  RETURN_IF_ERROR(DataTypeCheckForCpuBackend(qnn_model_wrapper, input_type, "QNN GroupNorm only supports float input for CPU backend."));
  ...
  const uint32_t num_channels = (node_unit.Domain() == "com.microsoft.nhwc") ? input_shape.back() : input_shape[1];
  ...
  if (node_unit.Domain() == "com.microsoft.nhwc") {
