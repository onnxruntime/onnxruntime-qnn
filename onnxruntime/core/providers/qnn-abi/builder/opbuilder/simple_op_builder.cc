...
Ort::Status SimpleOpBuilder::ExplicitOpCheck(QnnModelWrapper& qnn_model_wrapper, const OrtNodeUnit& node_unit) const {
  ...
  if (op_type == "DequantizeLinear") {
    ...
    if (qnn_model_wrapper.GetModelSettings().offload_graph_io_quantization &&
      ...
      // Register alias: quantized name -> original ONNX tensor name
      qnn_model_wrapper.SetTensorNameAlias(node_unit.Inputs()[0].name,
                                           node_unit.Outputs()[0].name);
  ...
  if (op_type == "QuantizeLinear") {
    ...
    if (qnn_model_wrapper.GetModelSettings().offload_graph_io_quantization &&
      ...
      // Register alias: quantized name -> original ONNX tensor name
      qnn_model_wrapper.SetTensorNameAlias(node_unit.Outputs()[0].name,
                                           node_unit.Inputs()[0].name);
