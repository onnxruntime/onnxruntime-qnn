...
OrtStatus* ORT_API_CALL QnnEpFactory::CreateEpImpl(OrtEpFactory* this_ptr,
  ...
  *ep = nullptr;
  ...
  RETURN_IF_NOT_NULL(factory->ort_api.CloneSessionOptions(session_options, &so));

  std::unique_ptr<QnnEp> qnn_ep;
  try {
    const auto provider_prefix = GetProviderOptionPrefix(factory->ep_name_);

    const auto backend_type = provider_prefix + "backend_type";
    const auto backend_path = provider_prefix + "backend_path";
    int has_backend_type = 0;
    int has_backend_path = 0;
    RETURN_IF_NOT_NULL(factory->ort_api.HasSessionConfigEntry(so, backend_type.c_str(), &has_backend_type));
    RETURN_IF_NOT_NULL(factory->ort_api.HasSessionConfigEntry(so, backend_path.c_str(), &has_backend_path));
    ...
    if (!has_backend_type && !has_backend_path) {
      ...
      std::string current_path = GetDynamicLibraryLocationByAddress(
      ...
      RETURN_IF_NOT_NULL(factory->ort_api.AddSessionConfigEntry(so,
    ...
    factory->ort_api.ReleaseSessionOptions(so);
