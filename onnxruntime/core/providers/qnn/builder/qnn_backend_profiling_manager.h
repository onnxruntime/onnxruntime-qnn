...
// Holds the manager-global QAIRT profiling lock across an ORT-attributed operation.
class QnnProfilingScope {
 public:
  QnnProfilingScope() = default;
  explicit QnnProfilingScope(Qnn_ProfileHandle_t profile_handle)
  ...
  void Complete(profile::ProfilingInfo& profiling_info) const;
...
class QnnBackendProfilingManager {
 public:
  ...
  Ort::Status InitializeProfiling(const Ort::Logger& logger,
                                  ProfilingLevel override_level = ProfilingLevel::INVALID);
  ...
  // Initializes profiling metadata and acquires a scope for synchronous setup operations such as
  // graph composition, graph finalization, and context loading.
  Ort::Status CreateSetupProfilingScope(profile::ProfilingInfo& profiling_info,
                                        const std::string& graph_name,
                                        OrtProfilingOperation ort_profiling_operation,
                                        ProfilingMethodType method_type,
                                        QnnProfilingScope& profiling_scope);
  // Initializes execute profiling metadata and acquires the graph-execute scope. This also
  // performs lazy ORT profiling-handle initialization and preserves the execute lock until the
  // active ORT profiler drains the extraction at StopEvent().
  Ort::Status CreateGraphProfilingScope(profile::ProfilingInfo& profiling_info,
                                        const std::string& graph_name,
                                        OrtProfilingOperation ort_profiling_operation,
                                        ProfilingMethodType method_type,
  ...
  Ort::Status SetProfilingLevelETW(ProfilingLevel profiling_level_etw, const Ort::Logger& logger);
  Ort::Status ExtractBackendProfilingInfo(profile::ProfilingInfo& profiling_info, const Ort::Logger& logger);
 ...
 private:
  QnnProfilingScope AcquireProfilingScope(bool current_operation_has_ort_profiler,
                                          bool has_active_ort_consumer);
  void InitializeProfilingInfo(profile::ProfilingInfo& profiling_info,
                               const std::string& graph_name,
                               OrtProfilingOperation ort_profiling_operation,
  ...
  bool AppendOrtProfilingEvent(QnnProfile_EventId_t event_id,
                               const std::string& event_level,
                               bool use_extended_event_data,
