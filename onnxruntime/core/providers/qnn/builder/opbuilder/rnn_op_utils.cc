...
// Decide whether a QDQ GRU/StatefulGru group runs as a genuine quantized Gru or must be fp-degraded.
// HTP has two native quantized Gru configs (HtpOpDefSupplement): an INT8 combo
...
// 16-bit with no such widening, so LBR=0 stays native there. StatefulGru's BOOL_8 reset is excluded
// from these quantization eligibility checks and passes through unchanged in the fp fallback.
...
Ort::Status AddUnidirectionGRU(QnnModelWrapper& qnn_model_wrapper,
  ...
  // fp-fallback: run the whole generated GRU subgraph in fp32. input_tensor_infos and
  ...
  if (use_fp_fallback) {
    for (size_t i = 0; i < input_tensor_infos.size(); i++) {
      if (!reset.onnx_name.empty() && onnx_inputs[i].name == reset.onnx_name) {
        continue;
      }
      if (onnx_inputs[i].Exists()) {
  ...
  if (!reset.onnx_name.empty()) {
    ...
    const std::string y_h_name = needs_y_h_output
                                     ? use_fp_fallback
                                           ? utils::UniqueNameGenerator().New(onnx_outputs[1].name, "_gru_fp32")
                                           : onnx_outputs[1].name
                                     : utils::UniqueNameGenerator().New(node_unit, "_Y_h_" + direction);

    qnn_gru_input_names[0] = input_names[0];
    qnn_gru_input_names[13] = initial_h_name;
    ...
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(y_tw)), "Failed to add multi-time-step GRU Y output.");

    const Qnn_TensorType_t y_h_type = !use_fp_fallback && needs_y_h_output && qnn_model_wrapper.IsGraphOutput(y_h_name)
                                          ? QNN_TENSOR_TYPE_APP_READ
                                          : QNN_TENSOR_TYPE_NATIVE;
    QnnTensorWrapper y_h_tw(y_h_name, y_h_type, output_tensor_infos[1].qnn_data_type,
                            output_tensor_infos[1].quant_param.Copy(),
                            {1, batch_size, hidden_size});
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(y_h_tw)), "Failed to add multi-time-step GRU Y_h output.");

    RETURN_IF_NOT(qnn_model_wrapper.CreateQnnNode(
                      utils::UniqueNameGenerator().New(node_unit, "_multi_time_step_" + direction),
                      QNN_OP_PACKAGE_NAME_QTI_AISW,
                      QNN_OP_GRU,
                      std::move(qnn_gru_input_names),
                      {y_name, y_h_name},
                      std::move(param_names),
                      do_op_validation),
                  "Failed to create multi-time-step QNN GRU node.");

    if (onnx_outputs.size() > 0 && onnx_outputs[0].Exists()) {
      const std::string y_out_name = is_bidirection
                                         ? utils::UniqueNameGenerator().New(y_name, "_unsqueeze_" + direction)
                                     : use_fp_fallback
                                         ? utils::UniqueNameGenerator().New(onnx_outputs[0].name, "_gru_fp32")
                                         : onnx_outputs[0].name;
      RETURN_IF_ERROR(qnn_model_wrapper.AddReshapeNode(y_name,
                                                       y_out_name,
                                                       {seq_length, batch_size, hidden_size},
                                                       ...
                                                       do_op_validation,
                                                       false,
                                                       qnn_model_wrapper.IsGraphOutput(y_out_name)));
      uni_gru_output_names.emplace_back(y_out_name);
    } else {
      uni_gru_output_names.emplace_back("");
    }

    if (onnx_outputs.size() > 1 && onnx_outputs[1].Exists()) {
      uni_gru_output_names.emplace_back(y_h_name);
    } else {
      uni_gru_output_names.emplace_back("");
    }
    return Ort::Status();
...
Ort::Status AddUnidirectionLSTM(QnnModelWrapper& qnn_model_wrapper,
  ...
  const bool multi_time_steps_lstm =
      enable_htp_monolithic_lstm || reset_requires_multi_time_steps_lstm;
  ...
  // time_major
  // Multi-time-step QNN_OP_LSTM consumes the whole 3D sequence at once (time_major=true);
  // the unrolled path feeds one 2D timestep per cell node (time_major=false).
  RETURN_IF_ERROR(AddQnnScalar<bool>(qnn_model_wrapper, node_unit.Index(), node_unit.Name(), multi_time_steps_lstm,
  ...
  std::vector<std::string> qnn_lstm_input_names(multi_time_steps_lstm ? 25 : 24, null_tensor_name);
  if (multi_time_steps_lstm) {
  ...
  if (!multi_time_steps_lstm) {
    ...
    // Multi-time-step path: a single QNN_OP_LSTM node consumes/produces the whole 3D sequence
    // at once, relying on HTP's native monolithic LSTM kernel (QNN_HTP_GRAPH_CONFIG_OPTION_MONOLITHIC_LSTM).
    // Stateful reset, if present, is connected at input[24].
    std::vector<std::string> qnn_lstm_output_names = {
