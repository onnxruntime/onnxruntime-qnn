...
Ort::Status RotaryEmbeddingOpBuilder::IsOpSupported(QnnModelWrapper& qnn_model_wrapper,
  ...
  RETURN_IF_NOT(sin_shape.size() == 2 || sin_shape.size() == 3,
                "sin_cache must be rank 2 or 3");
  RETURN_IF_NOT(cos_shape.size() == sin_shape.size(),
  ...
  ORT_CXX_LOG(logger, ORT_LOGGING_LEVEL_VERBOSE, "RotaryEmbedding op validation successful");
  return Ort::Status();
}
...
Ort::Status RotaryEmbeddingOpBuilder::DecomposeRotaryEmbedding(QnnModelWrapper& qnn_model_wrapper,
  ...
  RETURN_IF_NOT(rotary_dim % 2 == 0 && rotary_dim <= head_size,
                "Invalid rotary_embedding_dim");

  const uint32_t rotary_half_dim = rotary_dim / 2;

  // Input names in ONNX order: [input, position_ids, cos_cache, sin_cache]
  std::string current_tensor = input_names[0];
  const std::string& position_ids_name = input_names[1];
  const std::string& cos_cache_name = input_names[2];
  const std::string& sin_cache_name = input_names[3];
  const bool has_position_ids = true;  // position_ids is always present

  // Step 1: Normalize layout to [B, S, NH, HS]
  std::string normalized_tensor;
  ...
  // Create StridedSlice for x_rotate: slice last dimension [0:rotary_dim]
  {
    ...
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(x_rotate_tensor)),
  ...
  if (rotary_dim < head_size) {
    ...
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(x_tail_tensor)),
  ...
  if (interleaved) {
    ...
    // x1 = x_rotate[..., 0::2]
    {
      ...
      RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(x1_tensor)),
    ...
    // x2 = x_rotate[..., 1::2]
    {
      ...
      QnnTensorWrapper x2_tensor(x2_name, QNN_TENSOR_TYPE_NATIVE,
                                 qnn_data_type, input_info.quant_param.Copy(),
                                 std::vector<uint32_t>(x1_x2_shape));
      RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(x2_tensor)),
  ...
  if (has_position_ids) {
    ...
    // position_ids_name is already defined above as input_names[1]
    ...
    // Gather cos_cache
    {
      ...
      RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(cos_gathered_tensor)),
    ...
    // Gather sin_cache
    {
      ...
      RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(sin_gathered_tensor)),
  ...
  RETURN_IF_NOT(qnn_model_wrapper.GetOnnxShape(inputs[2].shape, cos_current_shape),
                "Cannot get cos_cache shape");
  ...
  std::string cos_x1 = utils::UniqueNameGenerator().New(node_unit, "_cos_x1");
  {
    ...
    QnnTensorWrapper cos_x1_tensor(cos_x1, QNN_TENSOR_TYPE_NATIVE,
                                   qnn_data_type, input_info.quant_param.Copy(),
                                   std::vector<uint32_t>(x1_x2_shape));
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(cos_x1_tensor)),
  ...
  std::string sin_x2 = utils::UniqueNameGenerator().New(node_unit, "_sin_x2");
  {
    ...
    QnnTensorWrapper sin_x2_tensor(sin_x2, QNN_TENSOR_TYPE_NATIVE,
                                   qnn_data_type, input_info.quant_param.Copy(),
                                   std::vector<uint32_t>(x1_x2_shape));
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(sin_x2_tensor)),
  ...
  std::string real = utils::UniqueNameGenerator().New(node_unit, "_real");
  {
    ...
    QnnTensorWrapper real_tensor(real, QNN_TENSOR_TYPE_NATIVE,
                                 qnn_data_type, input_info.quant_param.Copy(),
                                 std::vector<uint32_t>(x1_x2_shape));
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(real_tensor)),
  ...
  std::string sin_x1 = utils::UniqueNameGenerator().New(node_unit, "_sin_x1");
  {
    ...
    QnnTensorWrapper sin_x1_tensor(sin_x1, QNN_TENSOR_TYPE_NATIVE,
                                   qnn_data_type, input_info.quant_param.Copy(),
                                   std::vector<uint32_t>(x1_x2_shape));
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(sin_x1_tensor)),
  ...
  std::string cos_x2 = utils::UniqueNameGenerator().New(node_unit, "_cos_x2");
  {
    ...
    QnnTensorWrapper cos_x2_tensor(cos_x2, QNN_TENSOR_TYPE_NATIVE,
                                   qnn_data_type, input_info.quant_param.Copy(),
                                   std::vector<uint32_t>(x1_x2_shape));
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(cos_x2_tensor)),
  ...
  std::string imag = utils::UniqueNameGenerator().New(node_unit, "_imag");
  {
    ...
    QnnTensorWrapper imag_tensor(imag, QNN_TENSOR_TYPE_NATIVE,
                                 qnn_data_type, input_info.quant_param.Copy(),
                                 std::vector<uint32_t>(x1_x2_shape));
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(imag_tensor)),
  ...
  if (interleaved) {
    ...
    // Concat along last dimension
    {
      ...
      RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(stacked_tensor)),
    ...
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(x_rotated_tensor)),
  ...
  if (rotary_dim < head_size) {
    ...
    RETURN_IF_NOT(qnn_model_wrapper.AddTensorWrapper(std::move(output_normalized_tensor)),
  ...
  ORT_CXX_LOG(logger, ORT_LOGGING_LEVEL_VERBOSE, "Successfully decomposed RotaryEmbedding into QNN ops");
  return Ort::Status();
}
