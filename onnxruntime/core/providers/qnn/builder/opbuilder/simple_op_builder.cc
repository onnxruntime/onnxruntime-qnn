...
class SimpleOpBuilder : public BaseOpBuilder {
 ...
 private:
  ...
  static constexpr std::array<std::string_view, 2> scatterelements_supported_reduction = {"none", "add", "mul", "max"};
};
...
Status ProcessScatterElementsAttributes(QnnModelWrapper& qnn_model_wrapper,
                                        const NodeUnit& node_unit,
                                        std::vector<std::string>& param_tensor_names) {
  NodeAttrHelper node_helper(node_unit);
...
Status SimpleOpBuilder::ProcessAttributesAndOutputs(QnnModelWrapper& qnn_model_wrapper,
  ...
  if (op_type == "ScatterElements") {
    ORT_RETURN_IF_ERROR(ProcessScatterElementsAttributes(qnn_model_wrapper, node_unit, param_tensor_names));
  }
