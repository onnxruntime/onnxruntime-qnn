...
class CustomOpBuilder : public BaseOpBuilder {
 public:
  CustomOpBuilder(const std::string& op_package) : BaseOpBuilder("CustomOpBuilder"), op_package_(op_package) {}
  ORT_DISALLOW_COPY_ASSIGNMENT_AND_MOVE(CustomOpBuilder);
...
void CreateCustomOpBuilder(const std::string& op_type, const std::string& op_package, OpBuilderRegistrations& op_registrations) {
  op_registrations.AddOpBuilder(op_type, std::make_unique<CustomOpBuilder>(op_package), true);
}

Status CustomOpBuilder::ProcessAttributesAndOutputs(QnnModelWrapper& qnn_model_wrapper,
