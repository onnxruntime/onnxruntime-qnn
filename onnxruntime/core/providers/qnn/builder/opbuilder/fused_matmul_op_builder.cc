...
class FusedMatMulOpBuilder : public BaseOpBuilder {
 ...
 private:
  ...
  // Helper method to create batch transpose permutation vector
  ...
  // Helper method to handle batch dimension transposition
...
Status FusedMatMulOpBuilder::ProcessInputs(QnnModelWrapper& qnn_model_wrapper, const NodeUnit& node_unit,
  ...
  // Get tensor info for inputs A and B
...
Status FusedMatMulOpBuilder::ProcessAttributesAndOutputs(QnnModelWrapper& qnn_model_wrapper,
  ...
  // Get tensor info for inputs A and B to determine their shapes
  ...
  // Handle batch dimension transposition if needed
  ...
  // Create output tensors and nodes based on alpha value
  if (alpha == 1.0f) {
    ...
    // Create MatMul output tensor with empty quant params
    // When an operator is decomposed, the newly created intermediate/temporary tensor
    // should not use the quant param of inputs or outputs
    ...
    // Create multiply node for alpha scaling
    ORT_RETURN_IF_NOT(qnn_model_wrapper.CreateQnnNode(
...
void FusedMatMulOpBuilder::CreateBatchTransposePermVector(const std::vector<uint32_t>& input_shape,
  ...
  // Clear and resize to exactly the number of dimensions we need
  perm.clear();
...
Status FusedMatMulOpBuilder::HandleBatchTranspose(QnnModelWrapper& qnn_model_wrapper,
                                                  ...
                                                  bool trans_mat,
                                                  bool do_op_validation) const {
  // Create a transposed tensor name
  ...
  std::vector<uint32_t> perm;
  // Pass the appropriate transpose parameter based on which input we're handling
