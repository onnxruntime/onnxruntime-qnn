...
Ort::Status NonMaxSuppressionOpBuilder::IsOpSupported(QnnModelWrapper& qnn_model_wrapper,
  ...
  RETURN_IF(center_point_box != 0 && center_point_box != 1,
            "NonMaxSuppression: center_point_box must be 0 (diag corners) or 1 (center point).");
...
Ort::Status NonMaxSuppressionOpBuilder::ProcessAttributesAndOutputs(QnnModelWrapper& qnn_model_wrapper,
  ...
  // center_point_box (ONNX INT attribute, default 0 → QNN uint32)
  uint32_t center_point_box = static_cast<uint32_t>(
      OrtNodeAttrHelper(node_unit).Get("center_point_box", int64_t{0}));
  ...
  RETURN_IF_ERROR(AddQnnScalar<uint32_t>(qnn_model_wrapper, node_unit.Index(), node_unit.Name(),
                                         center_point_box,
                                         QNN_OP_NON_MAX_SUPPRESSION_PARAM_CENTER_POINT_BOX,
                                         param_tensor_names));
