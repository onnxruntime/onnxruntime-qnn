...
Ort::Status RopeOpBuilder::IsOpSupported(QnnModelWrapper& qnn_model_wrapper,
  ...
  if (!IsNpuBackend(qnn_model_wrapper.GetQnnBackendType())) {
    return MAKE_EP_FAIL("QNN RotaryEmbedding is only supported on HTP backend");
  }
  ...
  if (inputs.size() == 4 && inputs[3].Exists()) {
...
Ort::Status RopeOpBuilder::DecomposeRotaryEmbedding(QnnModelWrapper& qnn_model_wrapper,
  ...
  RETURN_IF_NOT(qnn_model_wrapper.GetOnnxShape(inputs[1].shape, cos_current_shape),
                "Cannot get cos_cache shape");
