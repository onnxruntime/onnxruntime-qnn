...
Status RopeOpBuilder::DecomposeRotaryEmbedding(QnnModelWrapper& qnn_model_wrapper,
  ...
  // Validate dimension values don't exceed int32_t max for QNN operations
  ORT_RETURN_IF_NOT(batch_size <= std::numeric_limits<int32_t>::max() &&
                    seq_len <= std::numeric_limits<int32_t>::max() &&
                    num_heads_val <= std::numeric_limits<int32_t>::max() &&
                    rotary_dim <= std::numeric_limits<int32_t>::max(),
                    "Dimension values exceed maximum supported range for QNN operations");

  // Validate position_ids data type if present
  const bool has_position_ids = (input_names.size() == 4);
  if (has_position_ids && inputs[3].node_arg.Exists()) {
    ...
    // Validate position_ids data type
    TensorInfo pos_ids_info = {};
    ORT_RETURN_IF_ERROR(qnn_model_wrapper.GetTensorInfo(inputs[3], pos_ids_info));
    ORT_RETURN_IF_NOT(pos_ids_info.qnn_data_type == QNN_DATATYPE_INT_32 ||
                      pos_ids_info.qnn_data_type == QNN_DATATYPE_INT_64,
                      "position_ids must be INT32 or INT64 data type");
  }
  ...
  const uint32_t expected_cache_dim = rotary_half_dim;
  ...
  // Validate cache sequence dimension is sufficient for 2D caches
  if (cos_shape.size() == 2) {
    // For 2D caches [max_seq_len, rotary_dim/2], verify max_seq_len >= input seq_len
    const uint32_t cache_max_seq_len = cos_shape[0];
    ORT_RETURN_IF_NOT(cache_max_seq_len >= seq_len,
                      "cos_cache sequence dimension must be >= input sequence length");
    ORT_RETURN_IF_NOT(sin_shape[0] >= seq_len,
                      "sin_cache sequence dimension must be >= input sequence length");
  }
