 ...
 */
Ort::Status CreateMatMulTransposeAll(
    onnxruntime::qnn::QnnModelWrapper* qnn_model_wrapper,
    const onnxruntime::OrtNodeUnit& node_unit,
    std::vector<std::string>&& input_names,
    bool do_op_validation) {
  onnxruntime::qnn::TensorInfo input_info0{}, input_info1{};
  RETURN_IF_ERROR(qnn_model_wrapper->GetTensorInfo(node_unit.Inputs()[0], input_info0));
  RETURN_IF_ERROR(qnn_model_wrapper->GetTensorInfo(node_unit.Inputs()[1], input_info1));
  ...
  RETURN_IF_NOT(qnn_model_wrapper->CreateQnnNode(/*qnn_node_name=*/onnxruntime::qnn::utils::UniqueNameGenerator().New(node_unit, QNN_OP_MAT_MUL),
                                                 ...
                                                 /*do_op_validation=*/do_op_validation),
                (node_unit.OpType() + " failed to add node.").c_str());
 ...
 * @return Status indicating success or failure of the operation.
 */
Ort::Status CreateReduceSumMulBroadcastX(
    onnxruntime::qnn::QnnModelWrapper* qnn_model_wrapper,
    const onnxruntime::OrtNodeUnit& node_unit,
    std::vector<std::string>&& input_names,
    bool do_op_validation) {
  ...
  RETURN_IF_ERROR(qnn_model_wrapper->GetTensorInfo(node_unit.Inputs()[0], tensor_info_in0));
  RETURN_IF_ERROR(qnn_model_wrapper->GetTensorInfo(node_unit.Inputs()[1], tensor_info_in1));
  RETURN_IF_ERROR(qnn_model_wrapper->GetTensorInfo(node_unit.Outputs()[0], tensor_info_out));
  ...
  const std::string mul_node_name =
      onnxruntime::qnn::utils::UniqueNameGenerator().New(node_unit, QNN_OP_ELEMENT_WISE_MULTIPLY);
  ...
  RETURN_IF_NOT(qnn_model_wrapper->AddTensorWrapper(std::move(tensor_wrapper_out)),
                "CreateReduceSumMulBroadcastX: failed to AddTensorWrapper");
...
Ort::Status EinsumOpBuilder::IsOpSupported(QnnModelWrapper& qnn_model_wrapper,
                                           const OrtNodeUnit& node_unit,
                                           const Ort::Logger& logger) const {
  if (node_unit.Inputs().size() < 2) {
    return MAKE_EP_FAIL((node_unit.OpType() + " requires at least 2 inputs.").c_str());
  }
  ...
  if (!parsed_equation.has_value()) {
    return MAKE_EP_FAIL((node_unit.OpType() + " unsupported equation: " + equation).c_str());
  }
  ...
  if (IsEquationReduceSumMulBroadcastX(parsed_equation.value())) {
    if (IsGpuBackend(qnn_model_wrapper.GetQnnBackendType())) {
      ...
      return MAKE_EP_FAIL((node_unit.OpType() + " unsupported equation: " + equation + " on backend GPU").c_str());
    }
    if (node_unit.Inputs()[0].quant_param.has_value()) {
      // QAIRT 3.36.1: Failed to finalize QNN graph 1002.
      return MAKE_EP_FAIL((node_unit.OpType() + " unsupported equation: " + equation + " for quantized inputs").c_str());
    }
  }
  return AddToModelBuilder(qnn_model_wrapper, node_unit, logger, true);
}

Ort::Status EinsumOpBuilder::ProcessInputs(QnnModelWrapper& qnn_model_wrapper,
                                           const OrtNodeUnit& node_unit,
                                           const Ort::Logger& logger,
                                           std::vector<std::string>& input_names,
...
Ort::Status EinsumOpBuilder::ProcessAttributesAndOutputs(QnnModelWrapper& qnn_model_wrapper,
                                                         const OrtNodeUnit& node_unit,
                                                         std::vector<std::string>&& input_names,
                                                         const Ort::Logger& logger,
                                                         bool do_op_validation) const {
  ...
  if (IsEquationMatMul(parsed_equation.value())) {
    ...
    RETURN_IF_ERROR(CreateMatMulTransposeAll(/*qnn_model_wrapper=*/&qnn_model_wrapper,
                                             /*node_unit=*/node_unit,
                                             /*input_names=*/std::move(input_names),
                                             /*do_op_validation=*/do_op_validation));
  } else if (IsEquationReduceSumMulBroadcastX(parsed_equation.value())) {
    RETURN_IF_ERROR(CreateReduceSumMulBroadcastX(/*qnn_model_wrapper=*/&qnn_model_wrapper,
                                                 /*node_unit=*/node_unit,
                                                 /*input_names=*/std::move(input_names),
                                                 /*do_op_validation=*/do_op_validation));
  } else {
...
Ort::Status EinsumOpBuilder::OverrideOutputQuantParam(QnnModelWrapper& qnn_model_wrapper,
                                                      const OrtNodeUnit& node_unit,
                                                      const Ort::Logger& logger,
                                                      const std::vector<std::string>& input_names,
                                                      size_t output_index,
