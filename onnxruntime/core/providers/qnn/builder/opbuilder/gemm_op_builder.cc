...
class GemmOpBuilder : public BaseOpBuilder {
 ...
 private:
  Status ExplictOpCheck(const OrtApi& ort_api, const OrtNodeUnit& node_unit) const;
};

Status GemmOpBuilder::ExplictOpCheck(const OrtApi& ort_api, const OrtNodeUnit& node_unit) const {
...
Status GemmOpBuilder::ProcessInputs(QnnModelWrapper& qnn_model_wrapper,
  ...
  if (do_op_validation) {
    ORT_RETURN_IF_ERROR(ExplictOpCheck(qnn_model_wrapper.GetOrtApi(), node_unit));
  }
