...
Ort::Status QnnBackendManager::GetQnnOpPackageInfo(const char* lib_path,
  ...
  GetInterfaceProvider = ResolveSymbol<QnnOpPackage_InterfaceProvider_t>(op_package_lib_handle, interface_name, *logger_ptr_);
  RETURN_IF(nullptr == GetInterfaceProvider, "Failed to get Op Package interface provider!");

  QnnOpPackage_Interface_t op_pkg_interface = QNN_OP_PACKAGE_INTERFACE_INIT;
  auto result = GetInterfaceProvider(&op_pkg_interface);
  ...
  // Check the op_pkg_interface's API version against the available versions.
  // Major versions must match. The op_pkg_interface's minor version must be greater OR equal with a suitable patch version.
  if (op_pkg_interface.interfaceVersion.major == op_package_api_version_1_4.major &&
      (op_pkg_interface.interfaceVersion.minor > op_package_api_version_1_4.minor ||
       (op_pkg_interface.interfaceVersion.minor == op_package_api_version_1_4.minor &&
        op_pkg_interface.interfaceVersion.patch >= op_package_api_version_1_4.patch))) {
    GetInfo = op_pkg_interface.v1_4.getInfo;
  } else if (op_pkg_interface.interfaceVersion.major == op_package_api_version_2_0.major &&
             (op_pkg_interface.interfaceVersion.minor > op_package_api_version_2_0.minor ||
              (op_pkg_interface.interfaceVersion.minor == op_package_api_version_2_0.minor &&
               op_pkg_interface.interfaceVersion.patch >= op_package_api_version_2_0.patch))) {
    GetInfo = op_pkg_interface.v2_0.getInfo;
  }
