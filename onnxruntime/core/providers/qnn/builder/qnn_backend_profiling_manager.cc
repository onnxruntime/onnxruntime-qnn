...
QnnProfilingScope QnnBackendProfilingManager::AcquireProfilingScope(
  ...
  if (!current_operation_has_ort_profiler) {
    ...
    if (!has_active_ort_consumer) {
      // Provider-only profiling retains unlocked execution behavior. The lock is
      // required only while an ORT event owns the shared handle and needs JSON event isolation.
...
Ort::Status QnnBackendProfilingManager::CreateGraphProfilingScope(
  ...
  if (!current_run_has_ort_profiler) {
    ...
    if (!has_active_ort_consumer) {
      // Keep provider-only profiling on its origin/main path. ORT-attributed operations hold the
      // lock from StartEvent through StopEvent so concurrent runs cannot mix JSON events.
  ...
  if (current_run_has_ort_profiler && profile_handle_ == nullptr) {
    // The handle may have been released after a previous ORT extraction.
