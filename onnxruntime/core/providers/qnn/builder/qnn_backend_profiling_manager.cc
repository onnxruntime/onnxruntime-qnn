...
QnnProfile_Error_t GetQnnProfileError(Qnn_ErrorHandle_t status) noexcept {
  return static_cast<QnnProfile_Error_t>(status & 0xFFFF);
}

bool IsQnnProfileSuccess(Qnn_ErrorHandle_t status) noexcept {
  return GetQnnProfileError(status) == QNN_PROFILE_NO_ERROR;
}
...
void QnnProfilingScope::Complete([[maybe_unused]] profile::ProfilingInfo& profiling_info) const {
...
Ort::Status QnnBackendProfilingManager::InitializeProfiling(const Ort::Logger& logger,
                                                            ProfilingLevel override_level) {
  ...
  if (merged_profiling_level_ == ProfilingLevel::OFF ||
    ...
    ORT_CXX_LOG(logger, ORT_LOGGING_LEVEL_INFO, "Profiling turned off.");
    return Ort::Status();
  }
  ...
  if (merged_profiling_level_ == ProfilingLevel::BASIC) {
    ORT_CXX_LOG(logger, ORT_LOGGING_LEVEL_VERBOSE, "Profiling level set to basic.");
  } else if (merged_profiling_level_ == ProfilingLevel::DETAILED) {
    qnn_profile_level = QNN_PROFILE_LEVEL_DETAILED;
    ORT_CXX_LOG(logger, ORT_LOGGING_LEVEL_VERBOSE, "Profiling level set to detailed.");
  } else if (merged_profiling_level_ == ProfilingLevel::OPTRACE) {
    qnn_profile_level = QNN_PROFILE_LEVEL_DETAILED;
    enable_optrace = true;
    ORT_CXX_LOG(logger, ORT_LOGGING_LEVEL_VERBOSE, "Profiling level set to optrace.");
  }
...
Ort::Status QnnBackendProfilingManager::InitializeProfilingForCurrentConsumers(const Ort::Logger& logger) {
  ...
  return InitializeProfiling(logger, override_level);
}
...
QnnProfilingScope QnnBackendProfilingManager::AcquireProfilingScope(
  ...
  const bool has_active_ort_consumer =
      !current_operation_has_ort_profiler && HasActiveOrtProfilingConsumer();
  return AcquireProfilingScope(current_operation_has_ort_profiler, has_active_ort_consumer);
}

QnnProfilingScope QnnBackendProfilingManager::AcquireProfilingScope(
    bool current_operation_has_ort_profiler,
    bool has_active_ort_consumer) {
  if (!current_operation_has_ort_profiler) {
    if (has_active_ort_consumer && !ProviderProfilingOutputActive()) {
    ...
    if (!has_active_ort_consumer) {
      // Provider-only collection has no ORT event ownership to isolate. It may use the shared
      // handle without the profiler-owned lock because no ORT JSON event can be mixed with it.
      return QnnProfilingScope(profile_handle_);
    }
  }
...
void QnnBackendProfilingManager::InitializeProfilingInfo(
    profile::ProfilingInfo& profiling_info,
    const std::string& graph_name,
    OrtProfilingOperation ort_profiling_operation,
    [[maybe_unused]] ProfilingMethodType method_type,
  ...
  if (ort_profiler != nullptr) {
    ...
    profiling_info.ort_profiling_operation = ort_profiling_operation;
  }

  if (scope_active && profiling_info.graph_name.empty()) {
    profiling_info.graph_name = graph_name;
  }
#ifdef QNN_SYSTEM_PROFILE_API_ENABLED
  if (scope_active) {
    profiling_info.start_time = qnn::utils::GetTimeStampInUs();
    profiling_info.method_type = method_type;
  }
#endif
}

Ort::Status QnnBackendProfilingManager::CreateSetupProfilingScope(
    profile::ProfilingInfo& profiling_info,
    const std::string& graph_name,
    OrtProfilingOperation ort_profiling_operation,
...
#if QNN_ORT_EP_PROFILING_API_ENABLED
  ort_profiler = QnnEpProfiler::Current();
#endif

  profiling_scope = AcquireProfilingScope(ort_profiler != nullptr);
  InitializeProfilingInfo(profiling_info, graph_name, ort_profiling_operation, method_type,
                          ort_profiler, profiling_scope.Active());
  return Ort::Status();
}
...
Ort::Status QnnBackendProfilingManager::CreateGraphProfilingScope(
    ...
    QnnProfilingScope& profiling_scope) {
  profiling_scope = QnnProfilingScope{};
  const bool current_run_has_ort_profiler = ort_profiler != nullptr;
  if (!ProviderProfilingActive() && !current_run_has_ort_profiler) {
    return Ort::Status();
  }

  if (current_run_has_ort_profiler) {
    std::unique_lock<std::recursive_mutex> lock(profile_handle_mutex_);
    if (profile_handle_ == nullptr) {
      // ORT extraction consumes the current QAIRT event tree and resets the handle, so each active
      // ORT execute scope creates a fresh handle when necessary.
      RETURN_IF_ERROR(InitializeProfilingForCurrentConsumers(logger));
    }
    if (profile_handle_ == nullptr) {
      return Ort::Status();
    }
  ...
  const bool has_active_ort_consumer = HasActiveOrtProfilingConsumer();
  if (!has_active_ort_consumer) {
    // Provider-only execution has no ORT event ownership to isolate, so it uses the profile handle
    // directly and records only the metadata needed by provider-output extraction.
    profiling_scope = QnnProfilingScope(profile_handle_);
  } else {
    // Work without a current ORT scope is excluded from ORT JSON. It remains eligible for a
    // configured provider sink, in which case scope acquisition serializes shared-handle access.
    QnnProfilingScope background_profiling_scope = AcquireProfilingScope(false, true);
    if (!background_profiling_scope.Active()) {
      return Ort::Status();
    }
    profiling_scope = std::move(background_profiling_scope);
  }
  InitializeProfilingInfo(profiling_info, graph_name, ort_profiling_operation, method_type,
                          ort_profiler, profiling_scope.Active());
  return Ort::Status();
...
Ort::Status QnnBackendProfilingManager::SetProfilingLevelETW(ProfilingLevel profiling_level_etw,
  ...
  status = InitializeProfiling(logger);
  if (!status.IsOK()) {
...
Ort::Status QnnBackendProfilingManager::ExtractBackendProfilingInfo(profile::ProfilingInfo& profiling_info,
                                                                    const Ort::Logger& logger) {
  std::unique_lock<std::recursive_mutex> lock(profile_handle_mutex_, std::defer_lock);
  if (profiling_info.ort_profiler != nullptr || HasActiveOrtProfilingConsumer()) {
    lock.lock();
  }
  if (merged_profiling_level_ == ProfilingLevel::OFF ||
      merged_profiling_level_ == ProfilingLevel::INVALID) {
  ...
  if (has_current_ort_profiler) {
    ...
    for (size_t event_idx = 0; event_idx < num_events; ++event_idx) {
      QnnProfile_EventId_t root_event_id = profile_events[event_idx];
      uint64_t root_qairt_timestamp_us = 0;
      if (backend_supports_extended_event_data) {
        QnnProfile_ExtendedEventData_t root_event = QNN_PROFILE_EXTENDED_EVENT_DATA_INIT;
        if (IsQnnProfileSuccess(qnn_interface_.profileGetExtendedEventData(root_event_id, &root_event)) &&
            root_event.version == QNN_PROFILE_DATA_VERSION_1) {
          root_qairt_timestamp_us = root_event.v1.timestamp;
        }
      }
      appended_event |= AppendOrtProfilingEvent(root_event_id, "ROOT", backend_supports_extended_event_data,
                                                root_qairt_timestamp_us, profiling_info);
    }
    if (!appended_event && profiling_info.ort_profiling_operation != OrtProfilingOperation::UNKNOWN) {
      profiling_info.ort_profiler->AppendHostOperationRecord(profiling_info.operation_start_time_us,
                                                             profiling_info.operation_end_time_us,
                                                             profiling_info.ort_profiling_operation,
                                                             profiling_info.graph_name);
    }
  }
#endif

  // A provider-only consumer may continue collecting into its handle. ORT extraction resets the
  // handle so the next ORT scope cannot replay events already assigned to this scope.
...
Ort::Status QnnBackendProfilingManager::ExtractProfilingSubEvents(
  ...
  const auto error_code = GetQnnProfileError(result);
  ...
  for (size_t sub_event_idx = 0; sub_event_idx < num_sub_events; ++sub_event_idx) {
    ...
    RETURN_IF_ERROR(ExtractProfilingSubEvents(subevent_id, profile_writer, use_extended_event_data, logger));
  }
  ORT_CXX_LOG(logger,
              ORT_LOGGING_LEVEL_VERBOSE,
...
#if QNN_ORT_EP_PROFILING_API_ENABLED
bool QnnBackendProfilingManager::AppendOrtProfilingEvent(
  ...
  if (use_extended_event_data) {
    QnnProfile_ExtendedEventData_t event_data = QNN_PROFILE_EXTENDED_EVENT_DATA_INIT;
    if (IsQnnProfileSuccess(qnn_interface_.profileGetExtendedEventData(event_id, &event_data)) &&
      ...
      record.args.push_back({"qnn_graph_name", profiling_info.graph_name});
    ...
    QnnProfile_EventData_t event_data = QNN_PROFILE_EVENT_DATA_INIT;
    if (IsQnnProfileSuccess(qnn_interface_.profileGetEventData(event_id, &event_data))) {
      event_read = true;
  ...
  if (event_read) {
    ...
    profiling_info.ort_profiler->AppendQnnEventRecord(std::move(record), root_qairt_timestamp_us, profiling_info);
    appended_event = true;
  }

  const QnnProfile_EventId_t* sub_events = nullptr;
  uint32_t num_sub_events = 0;
  if (IsQnnProfileSuccess(qnn_interface_.profileGetSubEvents(event_id, &sub_events, &num_sub_events))) {
    for (uint32_t i = 0; i < num_sub_events; ++i) {
      appended_event |= AppendOrtProfilingEvent(sub_events[i], "SUB-EVENT", use_extended_event_data,
                                                root_qairt_timestamp_us, profiling_info);
    }
  }
