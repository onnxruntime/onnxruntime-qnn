...
QnnProfilingScope QnnBackendProfilingManager::CreateScopedProfilingInfo(
    profile::ProfilingInfo& profiling_info,
    [[maybe_unused]] const std::string& graph_name,
    [[maybe_unused]] OrtProfilingOperation ort_profiling_operation,
  ...
  auto profiling_scope = AcquireProfilingScope(profiling_info.ort_profiler != nullptr);
  if (profiling_scope.Active() && profiling_info.graph_name.empty()) {
    profiling_info.graph_name = graph_name;
  }
#ifdef QNN_SYSTEM_PROFILE_API_ENABLED
...
#endif
  return profiling_scope;
}

Ort::Status QnnBackendProfilingManager::CreateGraphProfilingScope(
    bool current_run_has_ort_profiler, const Ort::Logger& logger, QnnProfilingScope& profiling_scope) {
