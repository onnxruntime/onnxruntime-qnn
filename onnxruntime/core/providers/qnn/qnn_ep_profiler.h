...
//  2. ORT guarantees a matching StopEvent for every StartEvent. It calls them around each profiled ORT node
//     on the run thread. StartEvent pushes a thread-local scope so QNN graph execution can find the current
//     profiler without cross-thread state.
...
class QnnEpProfiler : public OrtEpProfilerImpl {
 public:
  ...
  void AppendQnnEventRecord(EventRecord record,
 ...
 private:
  ...
  std::shared_ptr<ProfilerLifetimeToken> lifetime_token_;
