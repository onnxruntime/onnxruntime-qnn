...
struct ProfilerLifetimeToken {
  std::atomic<QnnEpProfiler*> profiler{nullptr};
};
...
struct OrtEventStart {
  std::shared_ptr<ProfilerLifetimeToken> lifetime_token;
  size_t index;
  int32_t thread_id;
...
const char* OrtProfilingOperationToString(onnxruntime::qnn::OrtProfilingOperation operation) noexcept {
  switch (operation) {
    case onnxruntime::qnn::OrtProfilingOperation::COMPOSE:
      return "compose";
    case onnxruntime::qnn::OrtProfilingOperation::FINALIZE:
      return "finalize";
    case onnxruntime::qnn::OrtProfilingOperation::CONTEXT_LOAD:
      return "context_load";
    case onnxruntime::qnn::OrtProfilingOperation::EXECUTE:
      return "execute";
    case onnxruntime::qnn::OrtProfilingOperation::UNKNOWN:
    default:
      return "";
  }
}
...
QnnEpProfiler::QnnEpProfiler(const OrtEpApi& ep_api,
    ...
    : OrtEpProfilerImpl{},
      ...
      lifetime_token_(std::make_shared<ProfilerLifetimeToken>()) {
  ...
  lifetime_token_->profiler.store(this, std::memory_order_release);
}

QnnEpProfiler::~QnnEpProfiler() {
...
void QnnEpProfiler::QueueExecuteProfilingExtraction(profile::ProfilingInfo profiling_info) {
  if (!tls_ort_event_start_indices.empty() &&
      tls_ort_event_start_indices.back().lifetime_token == lifetime_token_) {
...
Ort::Status QnnEpProfiler::DrainExecuteProfilingExtractions(std::vector<profile::ProfilingInfo> profiling_infos) {
  for (auto& profiling_info : profiling_infos) {
    RETURN_IF_ERROR(profiling_manager_.ExtractBackendProfilingInfo(profiling_info, logger_));
  }
  return Ort::Status();
}
...
QnnEpProfiler* QnnEpProfiler::Current() noexcept {
  while (!tls_ort_event_start_indices.empty()) {
    QnnEpProfiler* profiler = tls_ort_event_start_indices.back().lifetime_token->profiler.load(
...
void QnnEpProfiler::AppendQnnEventRecord(EventRecord record,
  ...
  const uint64_t event_time_us = profiling_info.operation_start_time_us + qairt_offset_us;
  ...
  const char* operation = OrtProfilingOperationToString(profiling_info.ort_profiling_operation);
  if (operation[0] != '\0') {
...
void QnnEpProfiler::AppendHostOperationRecord(uint64_t operation_start_time_us,
                                              ...
                                              OrtProfilingOperation operation,
                                              const std::string& graph_name) {
  const char* operation_name = OrtProfilingOperationToString(operation);
  EventRecord record;
...
OrtStatus* ORT_API_CALL QnnEpProfiler::StartEventImpl(OrtEpProfilerImpl* this_ptr,
  ...
  try {
    ...
    while (!tls_ort_event_start_indices.empty() &&
           tls_ort_event_start_indices.back().lifetime_token != self->lifetime_token_) {
      tls_ort_event_start_indices.pop_back();
    }
    tls_ort_event_start_indices.push_back({self->lifetime_token_, start_index, thread_id, {}, {}});
    return nullptr;
  } catch (const std::exception& e) {
    return self->ort_api_.CreateStatus(ORT_FAIL, e.what());
  } catch (...) {
