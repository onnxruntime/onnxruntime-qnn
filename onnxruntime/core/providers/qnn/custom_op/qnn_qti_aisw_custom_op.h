...
#include "core/providers/qnn/custom_op/qnn_custom_op.h"  // qnn::QnnUdoPlaceholderKernel (shared)
...
// varying arities of the ops without pinning exact type/count.
//
// The kernel is the shared qnn::QnnUdoPlaceholderKernel (Compute always errors — never executed
// for QNN-assigned nodes).
...
struct QtiAiswPlaceholderOp
  ...
  // Compute errors explicitly if such a node is ever executed (see QnnUdoPlaceholderKernel).
  ...
  OrtStatusPtr CreateKernelV2(const OrtApi& /*api*/, const OrtKernelInfo* /*info*/,
    ...
    *op_kernel = std::make_unique<QnnUdoPlaceholderKernel>(name_ ? name_ : "").release();
    return nullptr;
  }
