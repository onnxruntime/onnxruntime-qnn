...
bool WhereDummyDq::SatisfyCondition(const Graph& graph, const Node& node) const {
  ...
  //
  // When one `Where` branch is a scalar initializer (no producer node), QNN may require both
  // data branches to be produced by DQ nodes so the `Where` can be grouped into a single QDQ
  // node-unit. We insert a "dummy" DQ for the scalar branch to satisfy that requirement.
