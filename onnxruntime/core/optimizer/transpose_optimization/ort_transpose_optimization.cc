...
static bool EPAwareHandleReshape(HandlerArgs& args) {
  ...
  if (ep_type == kQnnExecutionProvider) {
    ...
    // Determine "channel" Transpose by checking perm being channel-fisrt to channel-last or vice versa.
    ...
    if (args.perm == perm_3d) {
      // There is ambiguity to determine the direction soley from this Transpose perm.
    ...
    for (size_t axis = 0; axis < output_rank; ++axis) {
      new_shape.push_back(output_shape[output_perm[axis]]);
    }
