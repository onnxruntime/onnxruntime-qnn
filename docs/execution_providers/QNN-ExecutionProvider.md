...
| `ReduceMean → Sub → Pow(2) → ReduceMean → Add(ε) → Sqrt → Div → Mul(γ) → Add(β)` | `QNN_OP_LAYER_NORM` | Matches the manual LayerNorm decomposition. Gamma and beta must be constants. Non-trailing axes (e.g., channel-first NCHW with `axes={1}`) are handled by inserting Transpose nodes around the LayerNorm op to move the normalized dims to trailing position. |
