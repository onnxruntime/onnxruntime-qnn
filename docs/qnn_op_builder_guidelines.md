...
- **Name intermediate *tensors*** with `utils::NodeUnitBaseName(node_unit) + "_suffix"` (deterministic). Avoid `UniqueNameGenerator` for tensor names — it is a stateful global counter that produces different names in GetCapability vs. Compile, causing the second pass to create a duplicate instead of reusing the first-pass tensor (see §12.5 footgun). `UniqueNameGenerator` is acceptable for QNN *node* names, which are not cross-referenced between passes. Note: many existing builders pre-date this guideline and still use `UniqueNameGenerator` for tensor names; those are known issues to be cleaned up incrementally.
