...
parser.add_argument('--genie-log-level', type=str, default='error', choices=['error', 'warn', 'info', 'verbose'], help='Genie log level (default: error)')
parser.add_argument('--input-shape', type=int, nargs='+', help='Custom input shape (e.g., --input-shape 1 128). If not provided, uses shape from model with dynamic dims set to 1')
parser.add_argument('--vocab-size', type=int, help='Vocabulary size for generating random token IDs')
parser.add_argument('--input-tokens-file', type=str, help='Path to file containing input tokens (one token per line or space/comma separated)')
...
def load_tokens_from_file(file_path):
    ...
    for line in content.split('\n'):
        line = line.strip()
        if ',' in line:
            tokens.extend([int(t.strip()) for t in line.split(',') if t.strip()])
        else:
            tokens.extend([int(t.strip()) for t in line.split() if t.strip()])
...
# EP-specific options
ep_options = {
                "genie_log_level": args.genie_log_level
            }
